...
public class GetAllUsersQueryHandler(IUserRepository userRepo) : IRequestHandler<GetAllUsersQuery, PaginatedResult<GetAllUserResult>>
{
  ...
  public async Task<PaginatedResult<GetAllUserResult>> Handle(
    GetAllUsersQuery request, CancellationToken cancellationToken)
  {
    ...
    var usersContent = users.Content.Select(user => new GetAllUserResult
    {
      Id = user.Id,
      Name = user.Name,
      Email = user.Email!,
    ...
    return new PaginatedResult<GetAllUserResult>(usersContent, users.TotalCount, users.PageNumber, users.PageSize);
  }
}
